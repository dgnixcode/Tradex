...
export class PlanningService {
  ...
  async preview(req: PlanRequest): Promise<PreviewResult> {
    ...
    const trade: NewGroupTrade = {
      ...
      stopLossPrice: (req.stopLossPrice && Number(req.stopLossPrice) > 0) ? req.stopLossPrice : null,
      takeProfitPrice: (req.takeProfitPrice && Number(req.takeProfitPrice) > 0) ? req.takeProfitPrice : null,
